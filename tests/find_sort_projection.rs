...
async fn sort_and_projection_pushdown() {
    ...
    let res = store
        .find_docs("test", "items", None, Some(&sort), Some(&projection), 10)
        .await
        .unwrap();
    ...
    for d in res {
        assert!(
            d.get_document("_id").is_ok()
                || d.get_object_id("_id").is_ok()
                || d.contains_key("_id")
        );
