...
async fn nested_projection_include_and_exclude() {
    ...
    store.insert_one("test", "docs", &id, &bson::to_vec(&d).unwrap(), &serde_json::to_value(&d).unwrap()).await.unwrap();

    // Include nested a.b and name, exclude _id
    let res = store.find_docs("test", "docs", None, None, Some(&doc!{"a.b": 1, "name": 1, "_id": 0}), 10).await.unwrap();
    assert_eq!(res.len(), 1);
    let r = &res[0];
    ...
    let res = store.find_docs("test", "docs", None, None, Some(&doc!{"a.c": 0}), 10).await.unwrap();
    let r = &res[0];
