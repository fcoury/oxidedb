...
async fn insert_and_find_round_trip_bson() {
    ...
    let n = store.insert_one("test", "users", &id_bytes, &bson_bytes, &json).await.expect("insert_one");
    assert_eq!(n, 1);

    // Find by id
    let found = store.find_by_id_docs("test", "users", &id_bytes, 10).await.expect("find_by_id");
