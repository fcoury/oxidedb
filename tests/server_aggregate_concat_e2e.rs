...
async fn e2e_aggregate_concat() {
    ...
    let dbname = format!("agg_concat_{}", rand_suffix(6));
    let create = doc!{"create": "u", "$db": &dbname};
    let msg = encode_op_msg(&create, 0, 1);
    stream.write_all(&msg).await.unwrap();
    let _ = read_one_op_msg(&mut stream).await;
    let ins = doc!{"insert": "u", "documents": [ {"_id":"a", "a":"Hello", "b":"World"} ], "$db": &dbname};
    let msg = encode_op_msg(&ins, 0, 2);
    stream.write_all(&msg).await.unwrap();
    let _ = read_one_op_msg(&mut stream).await;

    let proj = doc!{"msg": {"$concat": ["$a", " ", "$b"]}, "_id": 0i32};
    ...
    assert_eq!(d.get_str("msg").unwrap(), "Hello World");
