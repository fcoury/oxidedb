...
async fn e2e_unwind_include_array_index() {
    ...
    let ins =
        doc! {"insert": "u", "documents": [ {"_id":"d1", "tags":["b","a","c"]} ], "$db": &dbname};
    ...
    let fb = doc
        ...
        .unwrap();
    let pairs: Vec<(String, i32)> = fb
        .iter()
        .map(|b| {
            let dd = b.as_document().unwrap();
            (
                dd.get_str("tags").unwrap().to_string(),
                dd.get_i32("idx").unwrap(),
            )
        })
        .collect();
    ...
    assert_eq!(
        pairs,
        vec![("a".into(), 1), ("b".into(), 0), ("c".into(), 2)]
    );
